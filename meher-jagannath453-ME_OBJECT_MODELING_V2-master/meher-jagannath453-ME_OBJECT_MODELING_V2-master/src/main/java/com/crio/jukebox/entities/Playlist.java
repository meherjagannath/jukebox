...
public class Playlist extends BaseEntity {
    ...
    public boolean checkIfSongAlreadyPresent(List<Song> songs) {
        ...
        for (Song song : songs) {
            Song singleSong = this.songs.stream().filter(s -> s.getId().equals(song.getId()))
                    .findAny().orElse(null);
        ...
        if (songsPresentInPlaylist.size() > 0) {
            throw new SongAlreadyPresentInPlaylistException(
                    "Some songs are already present in the playlist.");
        }
    ...
    public boolean checkIfSongNotPresent(List<Song> songs) {
        ...
        if (songsNotPresentInPlaylist.size() > 0) {
            throw new SongNotPresentInPlaylistException(
                    "Some songs are not present in the playlist.");
        }
